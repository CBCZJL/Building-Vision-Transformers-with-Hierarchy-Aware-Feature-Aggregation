...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--data-path', default='/data/cyj', type=str,
                        help='dataset path')
    ...
    parser.add_argument('--output_dir', default='/home/cyj/NAS/PVT-2/result/pvt_tiny_test',
    ...
    parser.add_argument('--logging_save_path', type=str, default='./')
    return parser
...
def main(args):
    ...
    logging.setup_logging(os.path.join(args.logging_save_path, f'YJformer_PVT_tiny_.log'))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if test_stats["acc1"] > max_accuracy and epoch > args.epochs-15:
            if args.output_dir:
                checkpoint_paths = [output_dir / f'checkpoint_max_{epoch}.pth']
                for checkpoint_path in checkpoint_paths:
                    if is_main_process():
                        logger.info(f'Save checkpoint: checkpoint_max_{epoch}.pth')
                        logger.info(f"Save Path : {checkpoint_path}")
                    utils.save_on_master({
                        'model': model_without_ddp.state_dict(),
                        'optimizer': optimizer.state_dict(),
                        'lr_scheduler': lr_scheduler.state_dict(),
