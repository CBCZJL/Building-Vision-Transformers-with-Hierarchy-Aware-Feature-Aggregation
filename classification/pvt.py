...
from tcformer_layers import Block, TCBlock, OverlapPatchEmbed, CTM, DCN, DeformablePatchMerging, Conv_downsample
...
class Block_yj(nn.Module):

    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention_yj(
            dim,
            num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
    ...
    def forward(self, inputs, H, W):
        ...
        # attn
        x = x + self.drop_path(self.attn(q_dict['x'], kv_dict['x'], H, W))

        # mlp
        q_dict['x'] = self.norm2(x)
        x = x + self.drop_path(self.mlp(q_dict['x']))
        q_dict['x'] = x
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(1, 3):
            patch_embed = DeformablePatchMerging(embed_dims[i-1], embed_dims[i], img_size=img_size if i == 0 else img_size // (2 ** (i + 1)))
            num_patches = patch_embed.num_patches if i != num_stages - 1 else patch_embed.num_patches + 1
            pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dims[i]))
            pos_drop = nn.Dropout(p=drop_rate)
        ...
        for i in range(3, num_stages):
            patch_embed = CTM(0.25, embed_dims[i-1], embed_dims[i])
            block = nn.ModuleList([Block_yj(
                dim=embed_dims[i], num_heads=num_heads[i], mlp_ratio=mlp_ratios[i], qkv_bias=qkv_bias,
                qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + j],
                norm_layer=norm_layer, sr_ratio=sr_ratios[i])
                for j in range(depths[i])])
        ...
        for i in range(self.num_stages):
            ...
            if i == 0:
                ...
                if i == 2:
                    ...
                    token_dict['init_grid_size'] = [14, 14]
                    token_dict['map_size'] = [14, 14]
                    token_dict['token_num'] = 196
            else:
                token_dict['x'] = x.flatten(2).transpose(1, 2)
                token_dict_q, token_dict_kv, (H, W) = patch_embed(token_dict)
                x = (token_dict_q, token_dict_kv)
