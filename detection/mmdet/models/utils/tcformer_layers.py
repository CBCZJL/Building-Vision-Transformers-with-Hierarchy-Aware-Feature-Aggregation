...
class CTM(nn.Module):
    def __init__(self, sample_ratio, embed_dim, dim_out, k=5, groups = 1, off_kernel = 5, offset_range_factor=None):
        ...
        #     nn.Conv2d(self.dim_out//self.groups, self.dim_out//self.groups, off_kernel, 4, off_kernel//2, groups=self.groups),
        #     LayerNormProxy(self.dim_out//self.groups),
        #     nn.GELU(),
        #     nn.Conv2d(self.dim_out//self.groups, 2, 1, 1, 0, bias=False)
        # )
        # self.conv_channel = nn.Sequential(
        #     nn.Conv2d(self.dim_out, self.dim_out, kernel_size=1, stride=1),
        ...
        # )
        # self.norm = nn.LayerNorm(self.dim_out)
        # self.pool2 = nn.MaxPool2d(4, stride=4, return_indices=True)
        # self.pool3 = nn.MaxPool2d(2, stride=4, return_indices=True)
        # self.uppool_stage2 = nn.MaxUnpool2d(4, stride=4)
        # self.uppool_stage3 = nn.MaxUnpool2d(2, stride=4)
    ...
    def forward(self, token_dict):
        ...
        x_kv = token_dict['x']
        #----------------------------------------------------
        x, x_3 = self.conv(token_dict) # 这里就不动了
        x = self.norm(x)
        # x_kv = self.norm1(x_kv)
        #----------------------------------------------------
        token_score = self.score(x) # 每个Token的channel作为输入得到的score
...
class Conv_downsample(nn.Module):
    ...
    # def extra_repr(self) -> str:
    #     return f"input_resolution={self.input_resolution}, dim={self.dim}"import math
import einops
from turtle import forward
import torch
import time
