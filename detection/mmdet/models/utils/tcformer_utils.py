...
def index_points(points, idx):
    ...
    new_points = points[batch_indices, idx, :] # points: batch_indices表示原本的batch索引，idx多少行
    return new_points
...
def token2map(token_dict):
    ...
    if N_init == N and N == H * W:
        ...
        return x.reshape(B, H, W, C).permute(0, 3, 1, 2).contiguous() # token->map
...
def map2token(feature_map, token_dict):
    ...
    idx_hw = get_grid_index(
        [H_init, W_init], [H, W], device=device)[None, :].expand(B, -1) # (2, 3136)

    idx_batch = torch.arange(B, device=device)[:, None].expand(B, N_init) # (2, 3136)
    ...
    if N_init < N * H * W:
        # use sparse matrix multiplication
        # Flops: B * N_init * (C+2)
        idx_token = idx_token + idx_batch * N # 对整个batch所有Token进行统一编号
        idx_hw = idx_hw + idx_batch * H * W # 这个也是对整个batch进行统一操作
        # idx_token是原始Token数在Batch上的统一索引 idx_hw是小Token数索引插值到大图的索引值，用来将小Token联系到大Token数
        indices = torch.stack([idx_token, idx_hw], dim=0).reshape(2, -1) # indices[0]是大token数的索引，indices[1]是小token数的索引
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            A = torch.sparse_coo_tensor(indices, value, (B * N, B * H * W)) # A是大token数到小Token数的value矩阵，(大Token数，小Token数),转移矩阵
...
def cluster_dpc_knn(token_dict, cluster_num, k=5, token_mask=None):
    ...
    with torch.no_grad():
        ...
        score = dist * density
        _, index_down = torch.topk(score, k=cluster_num, dim=-1) # index_down聚类中心的索引

        # assign tokens to the nearest center
        dist_matrix = index_points(dist_matrix, index_down) # 选出784个聚类中心
...
#         dist_matrix = torch.cdist(x, x) / (C ** 0.5) # 不同Token到Token之间的距离矩阵
...
#         # get local density
#         dist_nearest, index_nearest = torch.topk(dist_matrix, k=k, dim=-1, largest=False) # 最小前5的值与索引，
...
#         # get distance indicator
#         mask = density[:, None, :] > density[:, :, None] # 公式2第一行
#         mask = mask.type(x.dtype)
#         dist_max = dist_matrix.flatten(1).max(dim=-1)[0][:, None, None] # max后面索引0表示返回的是值不是索引
#         dist, index_parent = (dist_matrix * mask + dist_max * (1 - mask)).min(dim=-1) # 找出每行的最小值

#         # select clustering center according to score
#         score = dist * density # 结合起来选择聚类中心
#         _, index_down = torch.topk(score, k=cluster_num, dim=-1) # 最有可能成为聚类中心的k个token

#         # assign tokens to the nearest center
#         dist_matrix = index_points(dist_matrix, index_down) # 把聚类中心那K个Token索引出来了

#         idx_cluster = dist_matrix.argmin(dim=1) # 这个是聚类中心Token和所有Token中最近的那个索引,每一列属于哪一行类，也就是这里3136个Token已经分为k类

#         # make sure cluster center merge to itself
#         idx_batch = torch.arange(B, device=x.device)[:, None].expand(B, cluster_num)
#         idx_tmp = torch.arange(cluster_num, device=x.device)[None, :].expand(B, cluster_num)
#         idx_cluster[idx_batch.reshape(-1), index_down.reshape(-1)] = idx_tmp.reshape(-1) #不理解

#         idx_batch = torch.arange(B, device=x.device)[:, None]
#         idx = idx_cluster + idx_batch * cluster_num
#         one_matrix = token_dict['num_matrix']
#         num_matrix = torch.zeros_like(x[:, :cluster_num, 0]).reshape(B * cluster_num, 1)
#         num_matrix.index_add_(dim=0, index=idx.reshape(B * N),
#                         source=one_matrix.reshape(B * N, 1))
#         num_matrix = num_matrix.reshape(B, cluster_num)
#         num, indixes = torch.sort(num_matrix, 1, descending=True)
#     return idx_cluster, cluster_num, num_matrix 
...
#             idx_batch = torch.arange(B, device=x.device)[:, None]
#             idx = idx_cluster + idx_batch * cluster_num
#             one_matrix = torch.ones_like(x[:, :, 0])
#             num_matrix = torch.zeros_like(x[:, :cluster_num, 0]).reshape(B * cluster_num, 1)
#             num_matrix.index_add_(dim=0, index=idx.reshape(B * N),
#                             source=one_matrix.reshape(B * N, 1))
#             num_matrix = num_matrix.reshape(B, cluster_num)
...
#         else:
#             # 在小聚类数目下拿到mask
#             cluster_num_small = cluster_num // 2
#             dist_matrix = torch.cdist(x, x) / (C ** 0.5) # 不同Token到Token之间的距离矩阵
#             # get local density
#             dist_nearest, index_nearest = torch.topk(dist_matrix, k=k, dim=-1, largest=False) # 最小前5的值与索引，
...
#             # select clustering center according to score
#             score = dist * density # 结合起来选择聚类中心
#             _, index_down = torch.topk(score, k=cluster_num_small, dim=-1) # 最有可能成为聚类中心的k个token, index_down类别绝对索引

#             # assign tokens to the nearest center
#             dist_matrix_small = index_points(dist_matrix, index_down) # 把聚类中心那K个Token索引出来了

#             idx_cluster = dist_matrix_small.argmin(dim=1) # idx_cluster是index_down的索引

#             # make sure cluster center merge to itself
#             idx_batch = torch.arange(B, device=x.device)[:, None].expand(B, cluster_num_small)
#             idx_tmp = torch.arange(cluster_num_small, device=x.device)[None, :].expand(B, cluster_num_small)
#             idx_cluster[idx_batch.reshape(-1), index_down.reshape(-1)] = idx_tmp.reshape(-1) #不理解

#             idx = idx_cluster + idx_batch[:,1].reshape(B,1) * cluster_num_small
#             one_matrix = token_dict['num_matrix']
#             num_matrix = torch.zeros_like(x[:, :cluster_num_small, 0]).reshape(B * cluster_num_small, 1)
#             num_matrix.index_add_(dim=0, index=idx.reshape(B * N),
#                             source=one_matrix.reshape(B * N, 1))
#             num_matrix = num_matrix.reshape(B, cluster_num_small)
#             # num, indixes = torch.sort(num_matrix, 1, descending=True)
#             indixes = torch.argmax(num_matrix,dim=1)
            
#             #-------------------------------------------------------------------------------------------
#             #在正常聚类数目下用mask屏蔽无关区域
#             indixes = indixes.reshape(B, 1)
#             mask_ones = torch.ones_like(x[:, :, 0]) * indixes - idx_cluster
#             mask_index = torch.nonzero(mask_ones)
#             token_mask = torch.zeros_like(x[:, :, 0])
#             token_mask[mask_index[:,0], mask_index[:,1]] = 1
...
#             # select clustering center according to score
#             score = dist * density # 结合起来选择聚类中心
#             _, index_down = torch.topk(score, k=cluster_num, dim=-1) # 最有可能成为聚类中心的k个token, index_down类别绝对索引

#             # assign tokens to the nearest center
#             dist_matrix = index_points(dist_matrix, index_down) # 把聚类中心那K个Token索引出来了

#             idx_cluster = dist_matrix.argmin(dim=1) # idx_cluster是index_down的索引
...
def merge_tokens(token_dict, idx_cluster, cluster_num, token_weight=None):
    ...
    idx_batch = torch.arange(B, device=x.device)[:, None]
    idx = idx_cluster + idx_batch * cluster_num # idx虽然还是3136个，但是实际上索引很多重复的只有784种

    all_weight = token_weight.new_zeros(B * cluster_num, 1) #token_weight是3136个token的分数，all_weight是3136个token的分数分别在784类的总和分数
    all_weight.index_add_(dim=0, index=idx.reshape(B * N),
                          source=token_weight.reshape(B * N, 1)) # 虽然idx看起来有3136个，但是这里索引是有重复的，也就是其实也只有784个索引值
    all_weight = all_weight + 1e-6
    norm_weight = token_weight / all_weight[idx] # 每个Token的得分除以这个Token属于的类的总得分，如果一个Token的norm_weight的值为1也就是单独一类
    # norm_weight归一化之后的每个Token的分数
    # average token features
    x_merged = x.new_zeros(B * cluster_num, C) # 这里就只有784个了
    source = x * norm_weight
    x_merged.index_add_(dim=0, index=idx.reshape(B * N),
                        source=source.reshape(B * N, C).type(x.dtype)) # source就已经是加权之后的x了,也就是这里已经合并了
...
def vis_tokens(img, token_dict, edge_color=[255.0/255.0, 185/255.0, 230/255.0], edge_width=1):
    """Visualize tokens
    Return:
    ...
    color_map = F.avg_pool2d(img, kernel_size=16)
...
def map2token_draw(feature_map, token_dict, cam_idx):
    """Transform feature map to vision tokens. This function only
    works when the resolution of the feature map is not higher than
    the initial grid structure.
    ...
    if N_init < N * H * W:
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            all_weight = A @ torch.ones(
                [B * H * W, 1], device=device, dtype=torch.float32) + 1e-6
            value = value / all_weight[idx_token.reshape(-1), 0] # 同一类，同样的值，分布是按照idx_token的分布
    ...
    for i in range(len(area)):
        out[0][area[i]] = torch.tensor([1.,0.,0.])
    return out

def draw_tokens(img, token_dict, cam_idx = None, edge_color=[255.0/255.0, 185/255.0, 230/255.0], edge_width=1):
    """Visualize tokens
    Return:
        vis_img (Tensor[B, 3, H, W]): visualize result.
    ...
    token_color = map2token_draw(color_map, token_dict, cam_idx)
    tmp_dict = token_dict.copy()
    tmp_dict['map_size'] = [H, W]
    tmp_dict['x'] = token_color
    vis_img = token2map(tmp_dict)
    ...
    for i in range(14):
        for j in range(14):
            if vis_img[0][i][j] != 1. and vis_img[1][i][j] != 0.  and vis_img[2][i][j] != 0:
                vis_img[0][i][j] = 0.
                vis_img[1][i][j] = 0.
                vis_img[2][i][j] = 0.
    vis_img = torch.unsqueeze(vis_img, dim=0)
    vis_img = F.interpolate(vis_img, [H * 16, W * 16], mode='nearest')
    idx_map = F.interpolate(idx_map, [H * 16, W * 16], mode='nearest')
    ...
    edge_color = torch.tensor(edge_color, device=device, dtype=dtype)[None, :, None, None]
    # vis_img = vis_img * (~edge_map) + edge_color * edge_map
    vis_img = edge_color * edge_map + org_img + vis_img
    return vis_img